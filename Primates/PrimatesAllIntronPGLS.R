...
for (i in seq(0, nrow(Lengthdb)-1, by=24)){
  ...
  PrimatesLengths <- SubsetLength$pecies.Total.Intron.Length
...
#qqplots and lambda corrections
...
qq(sorted_corrected_pvalues, main = "Q-Q plot of correlation p-values", 
   pch = 18, col = "blue4", cex = 1.5, las = 1) #NOMINAL pvalues

qq(pvalues, main = "Q-Q plot of correlation p-values", 
   pch = 18, col = "blue4", cex = 1.5, las = 1) #NOMINAL pvalues

qq(pvalues_lambda_corrected, main = "Q-Q plot of correlation p-values", 
   pch = 18, col = "blue4", cex = 1.5, las = 1)


pQQ(pvalues_lambda_corrected, conf = 0.95)

gene_length_mls_candidates <- c("ENSG00000213889", "ENSG00000108797", "ENSG00000116062", "ENSG00000138829",
...
#################################################
#####Candidates model step regression testing####
#################################################
...
for (value in candidates){
  for (i in seq(0, nrow(Lengthdb)-1, by=24)){
    ...
    if (GenesID[1] == value){
      ...
      Total_Lengths <- log10(c(as.numeric(Lengths), as.numeric(PrimatesLengths)))
      LengthPrimatesDataframe <- data.frame(Total_SpeciesList, Total_GenesID, Total_Lengths)
      names(LengthPrimatesDataframe) <- c("Total_SpeciesList", "Total_GenesID", paste(value, "_Total_Lengths", sep=""))
      all_model <- cbind(all_model, LengthPrimatesDataframe)
      #Loop for changing common names to match and including longevities
...
#STEPWISE REGRESSION
total_model<- cbind(LengthPrimatesDataframe$MaxLifespan, all_model$ENSG00000108797_Total_Lengths,
                      all_model$ENSG00000116062_Total_Lengths,
                      all_model$ENSG00000138829_Total_Lengths,
                      all_model$ENSG00000146842_Total_Lengths,
                      all_model$ENSG00000125434_Total_Lengths,
                      all_model$ENSG00000119698_Total_Lengths,
                      all_model$ENSG00000172663_Total_Lengths,
                      all_model$ENSG00000269313_Total_Lengths,
                      all_model$ENSG00000158113_Total_Lengths,
                      all_model$ENSG00000159216_Total_Lengths,
                      all_model$ENSG00000139546_Total_Lengths,
                      all_model$ENSG00000127824_Total_Lengths,
                      all_model$ENSG00000147606_Total_Lengths,
                      all_model$ENSG00000143951_Total_Lengths,
                      all_model$ENSG00000162419_Total_Lengths,
                      all_model$ENSG00000155052_Total_Lengths,
                      all_model$ENSG00000155052_Total_Lengths,
                      all_model$ENSG00000119537_Total_Lengths,
                      all_model$ENSG00000048405_Total_Lengths,
                      all_model$ENSG00000164134_Total_Lengths,
                      all_model$ENSG00000109339_Total_Lengths,
                      all_model$ENSG00000076706_Total_Lengths,
                      all_model$ENSG00000039537_Total_Lengths,
                      all_model$ENSG00000117569_Total_Lengths,
                      all_model$ENSG00000122034_Total_Lengths,
                      all_model$ENSG00000138964_Total_Lengths,
                      all_model$ENSG00000212443_Total_Lengths,
                      all_model$ENSG00000131473_Total_Lengths,
                      all_model$ENSG00000196476_Total_Lengths,
                      all_model$ENSG00000136048_Total_Lengths)

row.names(total_model) <- c("Homo_sapiens", "Macaca_mulatta", "Tarsius_syrichta", "Otolemur_garnettii",
                                        "Saimiri_boliviensis", "Pan_troglodytes_troglodytes", "Nomascus_leucogenys", 
                                        "Microcebus_murinus", "Pongo_abelii", "Pan_paniscus", "Callithrix_jacchus",
                                        "Gorilla_gorilla_gorilla", "Propithecus_coquereli", "Macaca_fascicularis", 
                                        "Rhinopithecus_roxellana", "Mandrillus_leucophaeus", "Hapalemur_simus",
                                        ...
                                        "Rhinopithecus_bieti", "Chlorocebus_pygerythrus", "Papio_anubis",
                                        "Cercocebus_torquatus_atys", "Macaca_nemestrina")
x_vals <- sprintf("x%s",seq(1:30), collapse=", ")
colnames(total_model) <- c("y",paste(x_vals))
total_dat <- as.data.frame(total_model)

obj<-gls(as.numeric(y)~x1+x2+x3+x4+x5+x6+x7+x8+x9+x10+x11+x12+x13+x14+x15+x16+x17+x18+x19+x20+x21+x22+x23+x24+x25,
         correlation=
           tree.coorel,data=as.data.frame(total_model),method="ML")
...
#Limitation to use the 30 values at the same time
...
#Make BOOTSTRAPPING OF MULTIPLE PGLS tests
...
pvalues <- c()
gene_ids <- c()
list_of_bootstrap_vectors <- list()
#LOOP FOR EACH GENE
...
sampleMaxLifespan <- sample(as.numeric(LengthPrimatesDataframe$MaxLifespan),
                                                    size=length(as.numeric(LengthPrimatesDataframe$MaxLifespan)),replace=TRUE)
sampleGeneLength <- sample(as.numeric(LengthPrimatesDataframe$Total_Lengths),
                                                   size=length(as.numeric(LengthPrimatesDataframe$Total_Lengths)),replace=TRUE)
new_sample_df = data.frame(MaxLifespan=sampleMaxLifespan, GeneLength=sampleGeneLength)

row.names(new_sample_df) <- c("Homo_sapiens", "Macaca_mulatta", "Tarsius_syrichta", "Otolemur_garnettii",
                                        "Saimiri_boliviensis", "Pan_troglodytes_troglodytes", "Nomascus_leucogenys", 
                                        "Microcebus_murinus", "Pongo_abelii", "Pan_paniscus", "Callithrix_jacchus",
                                        "Gorilla_gorilla_gorilla", "Propithecus_coquereli", "Macaca_fascicularis", 
                                        "Rhinopithecus_roxellana", "Mandrillus_leucophaeus", "Hapalemur_simus",
                                        ...
                                        "Rhinopithecus_bieti", "Chlorocebus_pygerythrus", "Papio_anubis",
                                        "Cercocebus_torquatus_atys", "Macaca_nemestrina")
if (class(try(gls(sampleMaxLifespan ~ sampleGeneLength, 
                  correlation = tree.coorel, data = new_sample_df,
                  method = "ML"))) == "try-error"){
  print("Singular data")
  
}
else{
  pglsModel <- gls(sampleMaxLifespan ~ sampleGeneLength, 
                   correlation = tree.coorel, data = new_sample_df,
                   method = "ML")
  pv <- anova(pglsModel)[2,3]
  pvalues<- append(pvalues, pv)
  ...
  corrected_pvalues <- p.adjust(pvalues, method = "fdr", n=length(pvalues))
  sorted_corrected_pvalues <- sort(corrected_pvalues , decreasing = FALSE)
  list_of_bootstrap_vectors[[num]] <- sorted_corrected_pvalues}
...
for (vec in list_of_bootstrap_vectors){
  best <- names(vec[vec < 0.05])
  print(length(best))
  print(length(gene_length_total_candidates))
  if (setequal(best, gene_length_total_candidates)){
    print("one case")}
  else{
    print("nop")}
}
