...
for (i in seq(0, nrow(Lengthdb)-1, by=24)){
  ...
  LengthPrimatesDataframe$BMI <- log10(ordered_bodymass)
  ...
  if (class(try(gls(as.numeric(BMI) ~ as.numeric(Total_Lengths), 
    ...
    pglsModel <- gls(as.numeric(BMI) ~ as.numeric(Total_Lengths), 
                     data = LengthPrimatesDataframe, correlation = tree.coorel,
                     method = "ML")
