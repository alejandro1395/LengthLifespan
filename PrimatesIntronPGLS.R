...
Lengthdb <- read.csv("FirsIntronLengths.tsv", header = TRUE, sep = "\t")
Lengthdb <- Lengthdb[,-1]
print(Lengthdb[1:26,])
...
for (i in seq(0, nrow(Lengthdb)-1, by=24)){
  ...
  hum_len <- as.vector(SubsetLength$Human.First.Intron.Length[1])
  species_len <- as.vector(SubsetLength$Species.First.Intron.Length)
...
#COmpare gene length vector with first intron vector
...
#candidates <- names(best)

best <- "ENSG00000148204"
candidates <- best
par(mfcol=c(1,1), oma=c(1,1,0,0), mar=c(1,1,1,0),
...
for (value in candidates){
  ...
  for (i in seq(0, nrow(Lengthdb)-1, by=24)){
    ...
    if (GenesID[1] == value){
      print("ok")
      ver1 <- as.data.frame(SubsetLength)
      hum_len <- as.vector(SubsetLength$Human.First.Intron.Length[1])
      species_len <- as.vector(SubsetLength$Species.First.Intron.Length)
      PrimatesList <- SubsetLength$Species
      PrimatesGenesID <- SubsetLength$Species.Gene.ID
      ...
      names(LengthPrimatesDataframe) <- c("Total_SpeciesList", "Total_GenesID", "Total_Lengths")
      #Loop for changing common names to match and including longevities
      ver2 <- as.data.frame(LengthPrimatesDataframe)
  ...
  p[[num]] <- ggplot(as.data.frame(cbind(hPic, aPic)), aes(x = hPic, y = aPic)) + 
    geom_point() +  geom_smooth(method='lm',formula=y~x) + annotation_custom(grob)
  #abline(a = coef(pglsModel)[1], b = coef(pglsModel)[2])
  
  
  
  #WITHOUT CORRECTING BY PHILOGENY
  #grob <- grobTree(textGrob(paste("R = ", round(cor(candidate_gene$Total_Lengths, candidate_gene$MaxLifespan),2)), x=0.1,  y=0.95, hjust=0,
  # gp=gpar(col="blue", fontsize=13, fontface="italic")))
  
  
  #p[[num]] <- ggplot(candidate_gene, aes(x = Total_Lengths, y = MaxLifespan)) + 
  #geom_point() +  geom_smooth(method='lm',formula=y~x) + annotation_custom(grob)
  #abline(a = coef(pglsModel)[1], b = coef(pglsModel)[2])
...
ggplot(data = candidate_gene, aes(y=MaxLifespan, x=Total_Lengths)) + geom_point() +
 ggrepel::geom_label_repel(label = names(length_gen))
